...
export async function seedHospital(db: PrismaClient, tenantId: string, siteId: string, seedUser: SeedUser) {
  ...
  await db.workplaces.upsert({ where: { id: s.workplaceId }, create: { id: s.workplaceId, name: "Sunrise Hospital", type: "HOSPITAL", tenantId, siteId, updatedAt: now }, update: {} });
  ...
  await db.hospitalWard.upsert({ where: { id: s.wardId }, create: { id: s.wardId, tenantId, workplaceId: s.workplaceId, name: "General Ward", type: "GENERAL" }, update: {} });
  await db.hospitalBed.upsert({ where: { id: s.bedId }, create: { id: s.bedId, tenantId, workplaceId: s.workplaceId, wardId: s.wardId, code: "GEN-01", dailyRate: "1000.00" }, update: {} });
